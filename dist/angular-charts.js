    ...
    function link(scope, element, attrs) {
      ...
      function pieChart() {
        ...
        var arcs = path.append('path').style('fill', function (d, i) {
            return getColor(i);
          }).transition().duration(500).attr('d', arc).attr('class', 'arc');
        path.on('mouseover', function (d) {
          angular.element('<p class="ac-tooltip" style="' + tooltip + '"></p>').html(d.data.tooltip || d.data.y[0]).appendTo('body').fadeIn('slow').css({
            left: event.pageX + 20,
            top: event.pageY - 30
          });
...
angular.module("left", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("left",
    "<div class='ac-legend' style='float:left; width:25%;'>\n" +
    "</div>\n" +
